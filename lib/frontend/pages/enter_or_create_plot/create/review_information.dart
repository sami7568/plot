...
class ReviewInformation extends StatefulWidget {
  ...
  final String plotName, paypalid, paypalname, paypalemail;
  ...
  const ReviewInformation({Key key, this.paypalid, this.paypalemail, this.paypalname,
    this.sharedPrefData, this.signature, this.plotName, this.plotDescription, this.plotAddress, this.hostName, this.contactDetails, this.latitude, this.longitude, this.startDate, this.plotPrivacy, this.plotImage, this.canBid, this.minimumBidPrice, this.free, this.ticketLevelsAndPrices, this.paymentMethods}) : super(key: key);
    ...
    Timer(Duration(milliseconds: 300), () async{
      if (_reviewPlotDetailsKey.currentState.validate()) {
        try {
          ...
          var _wallet = _firestore.collection('wallet').doc(widget.sharedPrefData.authID);

          await  _wallet.get().then((doc){
            if(!doc.exists){
              _wallet.set({
                'total': 0.0,
                'withdraw': 0.0,
                'paypal': {
                  'name': widget.paypalname,
                  'email': widget.paypalemail,
                  'paypalid': widget.paypalid,
                },
              });
            }else if(doc.exists){
              if(doc.data()['paypal']['paypalid'] != widget.paypalid){
                _wallet.update({
                  'paypal': {
                    'name': widget.paypalname,
                    'email': widget.paypalemail,
          ...
          }).whenComplete(( ) => print('wallet created'));
          ///
