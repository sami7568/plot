...
class _HomeState extends State<Home> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      endDrawer: NavigationDrawer(sharedPrefData: widget.sharedPrefData),
      // using tertiary statements to setup TabBarView {bool statement ? if true do whatever is here : else do this }
      body:  widget.sharedPrefData.joinedPlot && widget.sharedPrefData.approved ? PageView(
        ...
        widget.sharedPrefData.joinedPlot && !widget.sharedPrefData.approved ? PageView(
          physics: NeverScrollableScrollPhysics(),
          controller: _pageController,
          children: <Widget>[WaitingForApproval(sharedPrefData: widget.sharedPrefData), MapPage()]
        ): PageView(
          physics: NeverScrollableScrollPhysics(),
          controller: _pageController,
          children: <Widget>[MapPage(), JoinOrCreatePlot(sharedPrefData: widget.sharedPrefData)]
        ), // Not Joined View
      drawer: NavigationDrawer(sharedPrefData: widget.sharedPrefData),
      bottomNavigationBar: SafeArea(
          child: widget.sharedPrefData.joinedPlot && widget.sharedPrefData.approved ? Container(
            ...
            widget.sharedPrefData.joinedPlot && !widget.sharedPrefData.approved ? Container(
            margin: EdgeInsets.all(16),
            child: BottomNavyBar(
              backgroundColor: Colors.transparent,
              itemCornerRadius: 0,
